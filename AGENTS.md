...
poll.  The six provider arms in `build_agent()` share identical structure
and are generated by a local `arm!` macro — each expands to a client
constructor call plus the common `finish_agent()` helper.
